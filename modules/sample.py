...
class GoodRowSampler:
    ...
    def sample_(self):
        ...
        # print(np.sign(d_minus), np.sign(d_plus))
    ...
    @ut.timer
    def sample_parallel(self, n_rows):
        """
        Args:
            n_rows: number of rows to sample
        """
        results = Parallel(n_jobs=-1)(delayed(self.sample_)() for _ in range(n_rows))
    ...
    def sample(self, n_rows):
        ...
        Args:
            n_rows: number of rows to sample
        """
        results = [self.sample_() for _ in range(n_rows)]
        return np.array(results)
