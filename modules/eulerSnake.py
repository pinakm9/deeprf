...
class EulerSnake(nn.Module):
    def __init__(self, D, D_r, B, alpha=0.9):
        super().__init__()
        self.D = D
        self.D_r = D_r
        self.B = B
    ...
    def forward(self, x):
        ...
        for i in range(self.B):
            y += self.outer[i](torch.tanh(self.alpha * self.inner_W[i](y) + (1-self.alpha)*self.inner_U[i](x)))
        return y
...
class DeepRF(chain.DeepRF):
    def __init__(self, D_r, B, alpha, L0, L1, Uo, beta, name='nn', save_folder='.'):
        ...
        self.net = EulerSnake(self.sampler.dim, D_r, B, alpha)
    ...
    def init_xy(self, x, y):
        ...
        with torch.no_grad():
            for i in range(self.net.B):
                ...
                z = self.net.alpha * (W_in @ x1 + bW_in[:, np.newaxis]) + (1.-self.net.alpha) * (U_in @ x + bU_in[:, np.newaxis])
                x1 += W @ np.tanh(z)
